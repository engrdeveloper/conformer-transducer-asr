#!/usr/bin/env python3
"""Train and evaluate a Conformer Transducer speech recognition model.

The pipeline supports Common Voice data, offline and streaming recognition,
and attention-based or attention-free Conformer encoders. Training uses the
RNN-T objective with optional auxiliary CTC and cross-entropy losses.

Run from the transducer directory:
...
import signal
# --- Requeue support: SIGUSR1 handling ---
SHOULD_REQUEUE = False

def _sigusr1_handler(signum, frame):
    global SHOULD_REQUEUE
    logger.info("SIGUSR1 received in train.py, will trigger checkpoint & exit at safe point")
    SHOULD_REQUEUE = True
...
class ASR(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if hasattr(self.tokenizer, "sp") and hasattr(self.tokenizer.sp, "decode_ids"):
                predicted_words = [
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            # For file logger (can include strings)
            epoch_stats_file = {
                "epoch": epoch,
                "lr": lr,
                "steps": steps,
            ...
            self.hparams.train_tensor_logger.log_stats(
                stats_meta=epoch_stats_tb,
                train_stats=self.train_stats,
                valid_stats=stage_stats,
            )

            self.hparams.train_logger.log_stats(
                stats_meta=epoch_stats_file,
                train_stats=self.train_stats,
                valid_stats=stage_stats,
            )
            ...
            if if_main_process():
                ...
                early_stop_file = Path(self.hparams.output_folder) / "early_stop_state.txt"

                self.best_valid_wer = float("inf")
                self.bad_epochs = 0
                if early_stop_file.exists():

                    content = early_stop_file.read_text().strip().split()

                    if len(content) == 2:
                        self.best_valid_wer = float(content[0])
                        self.bad_epochs = int(content[1])
                    else:
                        logger.warning(
                            "early_stop_state.txt is empty or malformed. Resetting early stopping state."
                        )

                if current_wer < self.best_valid_wer - min_delta:
                    self.best_valid_wer = current_wer
                    self.bad_epochs = 0

                    snapshot_dir = Path(self.hparams.output_folder) / "best_save_snapshot"

                    if snapshot_dir.exists():
                        shutil.rmtree(snapshot_dir)
                    
                    shutil.copytree(self.hparams.save_folder, snapshot_dir)

                    logger.info(f"Saved best save/ snapshot at WER={current_wer:.4f}")
                else:
                    self.bad_epochs += 1

                if if_main_process():
                    early_stop_file.write_text(f"{self.best_valid_wer} {self.bad_epochs}\n")

                if epoch >= min_epochs and self.bad_epochs >= patience:
            ...
            global SHOULD_REQUEUE
            if SHOULD_REQUEUE:
                logger.info("Requeue flag set (SIGUSR1). Exiting after epoch %d", epoch)
                os._exit(0)
    ...
    def on_evaluate_start(self, max_key=None, min_key=None):
        ...
        # ---- COPY CHECKPOINTS BEFORE AVERAGING ----
        if if_main_process():
            backup_dir = Path(self.hparams.output_folder) / "saved_best_ckpts"
            if not backup_dir.exists():
                backup_dir.mkdir(parents=True, exist_ok=True)

                for ckpt in ckpts:
                    dst = backup_dir / ckpt.path.name
...
def dataio_prepare(hparams, tokenizer=None):
    ...
    # If tokenizer is not passed, use the one from hparams
    ...
    def text_pipeline(wrd):
        yield wrd
        if hasattr(tokenizer, "sp") and hasattr(tokenizer.sp, "encode_as_ids"):
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning("Skipping experiment directory copy due to permission issue.")
    ...
    try:
        ...
        logger.info("Training stopped early. Continuing to testing.")

    # Testing
    os.makedirs(hparams["output_wer_folder"], exist_ok=True)

    snapshot_dir = Path(hparams["output_folder"]) / "best_save_snapshot"
    copied_flag = Path(hparams["output_folder"]) / "snapshot_restored.txt"

    if snapshot_dir.exists():
        if not copied_flag.exists():
            if os.path.exists(hparams["save_folder"]):
                shutil.rmtree(hparams["save_folder"])

            shutil.copytree(snapshot_dir, hparams["save_folder"])

            with open(copied_flag, "w") as f:
                f.write("snapshot restored\n")

            logger.info("Restored best save/ snapshot before testing.")
        else:
            logger.info(
                f"Found {copied_flag}. Snapshot was already restored. Skipping copy."
            )

    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        streaming = hparams.get("streaming", False)
        test_lc = hparams.get("test_left_context")
        test_cs = hparams.get("test_chunk_size")
        if streaming and test_lc is not None and test_cs is not None:
            asr_brain.hparams.test_wer_file = os.path.join(
                hparams["output_wer_folder"],
                f"online_wer_{k}_lc_{test_lc}_cs_{test_cs}.txt",
            )
            
        else:
            asr_brain.hparams.test_wer_file = os.path.join(
                hparams["output_wer_folder"], f"offline_wer_{k}.txt"
            )
        asr_brain.evaluate(
            test_datasets[k],
