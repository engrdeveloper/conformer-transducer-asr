"""Convert Common Voice TSV metadata into SpeechBrain CSV manifests.

The generated manifests contain audio paths, durations, speaker IDs, and
normalized transcripts for tokenizer and ASR training.
...
def process_line(
    ...
    if language == "en":
        clip_name = os.path.basename(audio_path_filename)          # e.g. common_voice_en_123456.mp3
        stem = os.path.splitext(clip_name)[0]                      # common_voice_en_123456
        clip_id = stem.split("_")[-1]                              # 123456
        subdir = clip_id[:5]                                       # 12345
        audio_path = os.path.join(data_folder, "clips", subdir, clip_name)
    else:
